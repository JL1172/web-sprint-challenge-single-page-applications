        ...
        <StyledDiv>
            <div>
                <form onSubmit={submit} id="pizza-form">
                    ...
                    <input data-cy ="name" onChange={change} value={formData.name}
                        id="name-input" name="name"
                        type="text" placeholder="name"
                    />
                    {formErrors.name && <p  data-cy ="nameErr" style={{ color: "red" }}>{formErrors.name}</p>}

                    <label  htmlFor="size-dropdown">Select Pizza Size</label>
                    <select data-cy ="size" value={formData.size} onChange={change} id="size-dropdown" name="size">
                        <option value="">--Select One--</option>
                        <option value="small">Small</option>
                        <option value="medium">Medium</option>
                        <option value="large">Large</option>
                    </select>
                    {formErrors.size && <p  data-cy ="sizeErr" style={{ color: 'red' }}>{formErrors.size}</p>}

                    <label htmlFor="topping1">Pepperoni</label>
                    <input  data-cy ="top1" type="checkbox" id="topping1"
                        checked={topping1}
                        name="topping1" onChange={change} value="pepperoni"
                    />

                    <label htmlFor="sausage">Sausage</label>
                    <input  data-cy ="top2"  type="checkbox" id="sausage" name="topping2"
                        checked={topping2}
                        value="sausage" onChange={change} />

                    <label htmlFor="bacon">Bacon</label>
                    <input  data-cy ="top3"  type="checkbox" id="bacon" name="topping3"
                        onChange={change} checked={topping3}
                        value="bacon" />

                    <label htmlFor="mushshrooms">Mushshrooms</label>
                    <input  data-cy ="top4"  type="checkbox" name="topping4" id="mushshrooms"
                        onChange={change} checked={topping4}
                        value="mushshrooms" />

                    <label htmlFor="special-text">Special instructions</label>
                    <input  data-cy ="special"  id="special-text" type="text" placeholder="special instructions"
                        name="special" value={special} onChange={change} />

                    <input disabled={disabled}  data-cy ="submit"  type="submit" id="order-button" value="Add to Order" />
                </form>
            </div>
        </StyledDiv>
    )
